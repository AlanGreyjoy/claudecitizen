import type { PrefabComponent, PrefabComponentType, PrefabKind } from './schema';
...
export const COMPONENT_REGISTRY: ComponentDef[] = [
  ...
  },
  {
    ...
    kinds: ['station'],
    marker: true,
    createDefault: () => ({ type: 'elevator', id: 'lift-1', targetFloor: 'lobby' }),
  ...
  },
  {
    ...
    kinds: ['station'],
    marker: true,
    createDefault: () => ({ type: 'hangar-pad', hangarId: 'bay-1', padIndex: 1 }),
  ...
  },
  {
    ...
    label: 'Collider',
    kinds: ALL_KINDS,
    createDefault: () => ({ type: 'collider', shape: 'box', size: { x: 1, y: 1, z: 1 } }),
  ...
  },
  {
    ...
    hint: 'Seated pilot pose. Entity position is the seat; forward (+Z) faces the nose.',
  },
  {
    type: 'ramp-interact',
    label: 'Ramp Interact',
    kinds: ['ship'],
    marker: true,
    createDefault: () => ({ type: 'ramp-interact', placement: 'outside', radius: 3 }),
...
export function searchComponents(
  ...
  return getComponentsForKind(kind).filter((def) => {
    ...
    return def.type.includes(needle) || def.label.toLowerCase().includes(needle);
  });
}
