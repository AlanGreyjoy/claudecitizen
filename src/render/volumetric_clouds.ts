...
export function createVolumetricCloudManager(
  ...
  const cloudsEffect = new CloudsEffect(camera, { resolutionScale: 0.8 }, atmosphere);
  ...
  cloudsEffect.shadow.cascadeCount = 3;
  cloudsEffect.shadow.mapSize.setScalar(512);
  ...
  aerialPerspectiveEffect.shadowSampleCount = 16;
  ...
  function resize(_width: number, _height: number): void {
    // Resize operations are now managed by the central composer in spike_renderer.ts
  }
