...
/**
 * In-world station AVMS panel via CSS3D.
 * Station group axes: x = -right, y = up, z = forward (same as ship).
 *
 * While interactive (panel open), the bezel is reparented to a flat screen-space
 * host for reliable pointer hits.
 */

/** CSS pixel width of `.sc-avms-bezel` — keep in sync with sc-ui.css. */
export const AVMS_TERMINAL_PANEL_CSS_WIDTH_PX = 960;
/** CSS pixel height of `.sc-avms-bezel`. */
export const AVMS_TERMINAL_PANEL_CSS_HEIGHT_PX = 540;

export interface AvmsTerminalScreenOptions {
  panelEl: HTMLElement;
  overlayParent?: HTMLElement;
}

export interface AvmsTerminalScreenHandle {
  attachTo(stationGroup: THREE.Object3D): void;
  setPowered(powered: boolean): void;
  setInteractive(interactive: boolean): void;
  setSpec(spec: StationAvmsMarker | null): void;
  sync(): void;
...
export function createAvmsTerminalScreen(
  options: AvmsTerminalScreenOptions,
): AvmsTerminalScreenHandle {
  ...
  overlay.className = "sc-avms-css3d";
  ...
  flatHost.className = "sc-avms-flat-host";
  ...
  let currentSpec: StationAvmsMarker | null = null;
  let powered = false;
  ...
  function applySpec(spec: StationAvmsMarker): void {
    currentSpec = spec;
    panelEl.style.width = `${AVMS_TERMINAL_PANEL_CSS_WIDTH_PX}px`;
    panelEl.style.height = `${AVMS_TERMINAL_PANEL_CSS_HEIGHT_PX}px`;

    const metersPerPx = spec.screenWidth / AVMS_TERMINAL_PANEL_CSS_WIDTH_PX;
  ...
  function syncCssFromAnchor(): void {
    ...
    const metersPerPx = currentSpec.screenWidth / AVMS_TERMINAL_PANEL_CSS_WIDTH_PX;
    cssObject.scale.setScalar(metersPerPx);
  }
  ...
  return {
    ...
    setSpec(spec: StationAvmsMarker | null) {
      if (!spec) {
        currentSpec = null;
        glass.visible = false;
        powered = false;
    ...
    dispose() {
      ...
      const host = document.getElementById("avms-terminal");
