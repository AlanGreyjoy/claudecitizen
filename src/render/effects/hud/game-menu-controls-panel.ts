...
import type { ControlsTab } from './game-menu-controls';
import type { GameMenuCaptureState } from './game-menu-controls-capture';

type DeviceBindingGroup = 'analog' | 'button';

export interface GameMenuPanelDeps {
  cancelCapture: () => void;
  controlsRoot: HTMLElement | null;
  getControlsStatus: () => string;
  getControlsTab: () => ControlsTab;
  getCaptureState: () => GameMenuCaptureState;
  getSettings: () => GameSettings;
  onControlsTabChange: (tab: ControlsTab) => void;
  resetControlsTab: (tab: ControlsTab) => void;
  startDeviceCapture: (
    profileId: DeviceProfileId,
    group: DeviceBindingGroup,
    action: DeviceButtonActionId | FlightAnalogControlId,
  ) => void;
  startKeyboardCapture: (action: KeyboardActionId) => void;
  updateDeviceProfile: (
    profileId: DeviceProfileId,
...
export function createGameMenuControlsPanel(deps: GameMenuPanelDeps) {
  ...
  function renderControlsSubnav(): HTMLElement {
    ...
    for (const tab of tabDefinitions) {
      ...
      button.classList.toggle('is-active', deps.getControlsTab() === tab.id);
      button.addEventListener('click', () => {
        deps.onControlsTabChange(tab.id);
      });
      tabs.append(button);
    }
    return tabs;
  ...
  function renderControlsStatus(): HTMLElement | null {
    if (!deps.getControlsStatus()) return null;
    return createElement('p', 'sc-game-menu-control-status', deps.getControlsStatus());
  }

  function renderKeyboardControls(): HTMLElement {
    const panel = createElement('div', 'sc-game-menu-control-page');
    const mouseKeyboard = deps.getSettings().input.mouseKeyboard;
    const captureState = deps.getCaptureState();
    ...
    for (const action of KEYBOARD_ACTIONS) {
      ...
      const bindButton = createTextButton(
        ...
        captureState.keyboardCaptureAction === action.id
          ? 'Press a key...'
          : formatKeyboardBinding(mouseKeyboard.bindings[action.id]),
      );
      bindButton.addEventListener('click', () => deps.startKeyboardCapture(action.id));
    ...
    reset.addEventListener('click', () => deps.resetControlsTab('mouseKeyboard'));
  ...
  function renderDeviceBindingButton(
    ...
    const captureState = deps.getCaptureState();
    const capturing =
      captureState.deviceCapture?.profileId === profileId &&
      captureState.deviceCapture.group === group &&
      captureState.deviceCapture.action === action;
    const button = createTextButton(
      'sc-game-menu-bind-btn',
      capturing ? 'Move input...' : formatDeviceBinding(binding),
    );
    button.addEventListener('click', () => deps.startDeviceCapture(profileId, group, action));
    return button;
  }
  ...
  function renderDeviceControls(profileId: DeviceProfileId): {
    ...
    reset.addEventListener('click', () => deps.resetControlsTab(profileId));
    actions.append(reset);
    ...
    const preview = createBindingAxisPreview(profileId, () => deps.getSettings().input[profileId]);
  ...
  function renderControlsPanel(
    onPreviewStart: (cleanup: (() => void) | null) => void,
  ): void {
    if (!deps.controlsRoot) return;
    deps.controlsRoot.replaceChildren();
    deps.controlsRoot.append(renderControlsSubnav());
    const status = renderControlsStatus();
    if (status) deps.controlsRoot.append(status);
    const controlsTab = deps.getControlsTab();
    if (controlsTab === 'mouseKeyboard') {
      deps.controlsRoot.append(renderKeyboardControls());
      onPreviewStart(null);
      return;
    }
    const { panel, startPreview } = renderDeviceControls(controlsTab);
    deps.controlsRoot.append(panel);
