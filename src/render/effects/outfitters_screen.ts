...
/**
 * In-world station outfitters panel via CSS3D.
...
/** CSS pixel width of `.sc-outfitters-bezel` — keep in sync with sc-ui.css. */
export const OUTFITTERS_PANEL_CSS_WIDTH_PX = 960;
/** CSS pixel height of `.sc-outfitters-bezel`. */
export const OUTFITTERS_PANEL_CSS_HEIGHT_PX = 540;

export interface OutfittersScreenOptions {
  panelEl: HTMLElement;
  overlayParent?: HTMLElement;
}

export interface OutfittersScreenHandle {
  attachTo(stationGroup: THREE.Object3D): void;
  setPowered(powered: boolean): void;
  setInteractive(interactive: boolean): void;
  setSpec(spec: StationOutfittersMarker | null): void;
  sync(): void;
...
export function createOutfittersScreen(
  options: OutfittersScreenOptions,
): OutfittersScreenHandle {
  ...
  overlay.className = "sc-outfitters-css3d";
  ...
  flatHost.className = "sc-outfitters-flat-host";
  ...
  let currentSpec: StationOutfittersMarker | null = null;
  let powered = false;
  ...
  function applySpec(spec: StationOutfittersMarker): void {
    currentSpec = spec;
    panelEl.style.width = `${OUTFITTERS_PANEL_CSS_WIDTH_PX}px`;
    panelEl.style.height = `${OUTFITTERS_PANEL_CSS_HEIGHT_PX}px`;

    const metersPerPx = spec.screenWidth / OUTFITTERS_PANEL_CSS_WIDTH_PX;
  ...
  function syncCssFromAnchor(): void {
    ...
    const metersPerPx = currentSpec.screenWidth / OUTFITTERS_PANEL_CSS_WIDTH_PX;
    cssObject.scale.setScalar(metersPerPx);
  }
  ...
  return {
    ...
    setSpec(spec: StationOutfittersMarker | null) {
      if (!spec) {
        currentSpec = null;
        glass.visible = false;
        powered = false;
    ...
    dispose() {
      ...
      const host = document.getElementById("outfitters");
