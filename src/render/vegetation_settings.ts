...
function normalizeLayer(
  ...
  const density = clamp(
    Number.isFinite(layer?.density) ? (layer?.density as number) : defaults.density,
    limits.density[0],
    limits.density[1],
  );
  const gapMeters = clamp(
    Number.isFinite(layer?.gapMeters) ? (layer?.gapMeters as number) : defaults.gapMeters,
    limits.gapMeters[0],
    limits.gapMeters[1],
  );
  const minScale = clamp(
    Number.isFinite(layer?.minScale) ? (layer?.minScale as number) : defaults.minScale,
    limits.scale[0],
    limits.scale[1],
  );
  const maxScale = clamp(
    Number.isFinite(layer?.maxScale) ? (layer?.maxScale as number) : defaults.maxScale,
    limits.scale[0],
    limits.scale[1],
  );
