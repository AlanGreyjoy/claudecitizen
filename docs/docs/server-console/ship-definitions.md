...
Definitions live in the PostgreSQL `ShipDefinition` table managed through SQLx. The Server console's **Ships** tab is the primary way to manage them.
...
The prefab dropdown is built from the project's `*.prefab.json` files where `kind === "ship"`. Adding a new flyable ship to the catalog requires:

1. Author the prefab in the [AsteronEngine editor](/editor) and save it anywhere under the project's `assets/` library.
