...
- Register definitions in the [Server console](/server-console/prop-definitions) to appear in the player catalog
...
Items without a prefab can use `iconUrl` in the Server console item definition — HUD renders the icon via `item-icon.ts`.
...
- List bundled item prefabs via `list-item-prefabs.ts` for the admin picker
...
```mermaid
flowchart LR
  Editor["AsteronEngine editor — save prefab JSON"]
  Disk["prefab JSON in repo"]
  Admin["Server console — definitions"]
...
Author geometry in the AsteronEngine editor, then create matching rows in the Server console so players can receive or purchase the content.
