...
sidebar_position: 3
title: Interface
description: AsteronEngine editor layout, panels, toolbar, scene tabs, and keyboard shortcuts.
---

# Interface

The AsteronEngine editor is a standalone, Unity-style Electron workspace with resizable
panels. Panel sizes persist in `localStorage`.

Launch it with `npm run editor:dev` for HMR, or `npm run editor` for a
production-like build. There is no separate Vite development server — Electron
owns Vite.
...
```text
┌─────────────────────────────────────────────────────────────┐
│ File/Edit/Play  │   ▶ Play ‖ Pause   │ active document      │
├──────────┬──────────────────────────────┬─────────────────────┤
...
The main Three.js viewport, shown when the **Scene** tab is active. The tab is
labelled **Prefab** while a prefab document is open.
...
| **Play / Pause / Stop** | Play Mode for the active scene, prefab, planet, system, or character test (`F6` / `F7` / `Shift+F6`) |
| **Build Web** | Save the active document and build the browser release into the project's configured output directory (`Ctrl+B`) |
| **Exit to Title** | Leave the open document and return to the title screen |
...
| **Server** | Operator console for the Rust backend — players, catalog, game settings |

Scenes and prefabs open from **File → Open Scene… / Open Prefab…**.

See [Material manager](./material-manager), [Planet authoring](./planet-authoring),
[System Map](./system-map), [Menu Manager](./menu-manager), and the
[Server console](/server-console).

Every tab except **Server** authors project files on disk. The Server tab is the
one place that edits **persistent server data** in PostgreSQL.
...
| Project Settings… | — |
| Save | `Ctrl+S` |
| Open Project… (back to the Projects hub) | `Ctrl+O` |
| Show Project Folder | — |
| Build Web | `Ctrl+B` |
| Exit to Title | — |

**Scene Settings…** edits the open scene document. **Project Settings…** edits
...
| `F7` | Pause / resume Play mode |
| `Shift+F6` | Stop Play mode |
| `Ctrl+O` | Open Project… (returns to the Projects hub) |
| `Ctrl+Z` | Undo |
