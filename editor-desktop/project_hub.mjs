  ...
  async function createProject({ name: rawName, parentDir }) {
    ...
    await writeFile(
      ...
      [
        ...
        '',
        '## Paths the engine looks for by convention',
        '',
        'Most content is found by reference — a prefab, planet, or animation',
        'controller names the URL, so the file can live anywhere. These few are',
        'looked up by fixed path instead, because engine code references them',
        'directly. All are optional; features degrade when a pack is absent.',
        '',
        '| Path under `assets/` | Used by |',
        '| --- | --- |',
        '| `protected/animations/universal-animation-library/UAL1_Standard.glb` | Retarget skeleton + default mannequin avatar |',
        '| `animations/ProRifle/locomotion.glb` | Default rifle locomotion pack (multi-clip) |',
        '| `animations/HandgunLocomotions/locomotion.glb` | Default pistol locomotion pack (multi-clip) |',
        '| `protected/characters/` | Built-in character avatar catalog |',
        '',
        'Prefer one multi-clip `locomotion.glb` per stance pack over per-clip GLBs.',
        'Merge loose exports with `npm run pack:anims -- --project <projectRoot>`',
        '(or `--in <clipDir> --out <locomotion.glb>`). Loose per-clip files can stay',
        'for re-pack; the animation controller references the pack URL.',
        '',
        'Licensed packs (Synty and similar) are deliberately not redistributed',
        'with the engine — supply your own copy at these paths.',
        '',
      ].join('\n'),
    );
